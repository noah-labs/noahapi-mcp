...
import { type PutCustomersByIdSchema, putCustomersByIdSchema } from "./schema";
import { noahClient } from "../../../utils/noah-client";

/**
 * Create/Update Customer
 */
export const putCustomersById = async (args: PutCustomersByIdSchema): Promise<string> => {
  try {
...
export const putCustomersByIdTool: ToolRegistration<PutCustomersByIdSchema> = {
  ...
  inputSchema: putCustomersByIdSchema,
  handler: async (args: PutCustomersByIdSchema) => {
    try {
      const parsedArgs = putCustomersByIdSchema.parse(args);
      const result = await putCustomersById(parsedArgs);
      return {
        content: [
          {
            type: "text",
      ...
      console.error("Error in putCustomersByIdTool handler:", error);
      return {
        content: [
          {
            type: "text",
